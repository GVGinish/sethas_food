         ...
         <div class="col-lg-12 cart_v2--cols">
            <ul class="cart_orders--wrapper">
               ...
               <li class="order_list" data-aos="fade-up">
                  ...
                  <div class="item_name">
                     {{$pro->product_name}} <br>
                     <span class="product_item--price cart-weight">{{$pro->weight}}</span>
                     <span class="product_item--price">₹ {{$pro->retail}}</span>
                     @php
                        $stock = App\Models\ProductModel::where('product_id', $pro->product_id)->value('stock');
                    @endphp

                    @php if ($stock == "Out of stock") { @endphp
            ...
            <ul class="order_list_total--wrapper" data-aos="fade-up" data-aos-delay="600">
               <li class="order_list--total">
                  ...
                  <div class="subtotal_wrapper">
                     ...
                     <span class="subtotal_price" id="cart_total">₹ {{array_sum($cart_default_total)}}</span>
                    </div>
               </li>
            </ul>
            <a class="btn btn-sm btn-primary" href="{{route('Checkout',['cart_id'=>$pro->cart_id])}}" id="checkout">Proceed To Checkout</a>
         </div>
      </div>
   </div>
</section>
...
<script>
   ...
   .done(function(res) {
     ...
     if (res.status === 'noquantity') {
         ...
         //$('#cart_total1').text(res.cart_total);

         $('#decreaseValue' + num).prop('disabled', false);
         $('#checkout').removeClass('disabled').off('click').attr('href', 'Checkout{{$pro->cart_id}}'); // Re-enable and set href
     }
   });
   }
